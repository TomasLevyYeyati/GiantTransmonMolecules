...
# ╔═╡ 4d58d1fd-0f41-4ba6-baa8-9da9b6843293
using Plots

# ╔═╡ c106f49e-51be-11f0-2b70-fdcbd5eaddc0
function Gplusplus(Γ,ω,J)
	(1+1im*Γ/2 ./(ω+J))./(ω-J+1im*Γ/2*(1+ (ω-J)./(ω+J)))
end;

# ╔═╡ 59deb5d9-9769-430a-8217-08906c92d120
function Gplusminus(Γ,ω,J)
	(-1im*Γ/2 ./(ω+J))./(ω-J+1im*Γ/2*(1+ (ω-J)./(ω+J)))
end;

# ╔═╡ 755c566c-7599-4ecd-9f93-47e750a0601e
function Aprox(Γ,ω,J)
	1 ./(ω-J+1im*Γ/2)
end;

# ╔═╡ f876d9f9-c8ba-4e3c-a371-781dd0adf784
begin
	Γ=1
	J=1*Γ
	plot(ω-> abs.(Gplusplus(Γ,ω,J)),-20*Γ,30*Γ)
	plot!(ω-> abs.(Gplusminus(Γ,ω,J)),-20*Γ,30*Γ)
	plot!(ω-> abs.(Aprox(Γ,ω,J)),-20*Γ,30*Γ) 
	J=10*Γ
	plot!(ω->abs.(Gplusplus(Γ,ω,J)),-20*Γ,30*Γ)
	plot!(ω->abs.(Gplusminus(Γ,ω,J)),-20*Γ,30*Γ)
	plot!(ω->abs.(Aprox(Γ,ω,J)),-20*Γ,30*Γ)
...
# ╔═╡ 658c3970-23f1-4309-ad18-9a25492b383d


# ╔═╡ a2113262-4fb0-4eea-aa78-2f78451c3f50
